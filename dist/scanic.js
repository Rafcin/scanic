...
function wA(I, A, Q, B, i, g) {
  ...
  for (; h++ < a; ) {
    ...
    if (E === -1) {
      ...
      for (let F = 0; F < 8; F++) {
        s = (i + F) % 8;
        const J = o.x + X[s].dx, c = o.y + X[s].dy;
    ...
    let G = null;
    for (let N = 0; N < 8; N++) {
      const F = (s + N) % 8, J = o.x + X[F].dx, c = o.y + X[F].dy;
      if (J >= 0 && J < A && c >= 0 && c < Q && I[c * A + J] > 0) {
        G = { x: J, y: c }, E = (F + 4) % 8;
        break;
      }
...
async function II(I, A = {}) {
  ...
  return Q.unshift({ step: "Total", ms: (Z - B).toFixed(2) }), console.table(Q), l;
}
const K = _(), v = {
...
async function BI(I, A = {}) {
  ...
  const Z = EI(l, Q, B);
  o.push({ step: "Hysteresis", ms: (performance.now() - h).toFixed(2) }), E && (E.beforePost = new Uint8Array(Z));
  let R = Z;
